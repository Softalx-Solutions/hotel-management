...
function NavBar() {
  return (
    <nav className="col-span-1 sm:col-span-1 bg-dark">
      <div className="my-6 mx-4 flex items-center">
        <img src={logo} alt="logo" />
        <h1 className="text-secondary font-bold ml-4 flex sm:hidden">
          <a href="/">Dashboard</a>
        </h1>
      </div>
      <ul className="mt-8">
        {links.map((link) => {
          return (
            <li className="flex p-3">
              {link.icon}
              <h3 className="flex sm:hidden text-[#DDE2FF] ml-4">{link.title}</h3>
            </li>
          );
        })}
        {/* Active */}
